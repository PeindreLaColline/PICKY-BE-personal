...
//    Slice<Board> findByMovieId(@Param("movid_id") Long movieId, Pageable pageable);
    ...
    @Query("""
    SELECT b.id AS boardId, b.userId AS boardWriterId, b.writerNickname AS boardWriterNickname, u.profileUrl AS userProfileUrl, b.context AS boardContext, b.isSpoiler AS isSpoiler,
    ...
    (CASE WHEN EXISTS (SELECT 1 FROM BoardLike bl WHERE bl.board.id = b.id AND bl.user.id = :userId) THEN true ELSE false END) AS likedByCurrentUser,
    ...
    """)
//    @Query("SELECT b.writerNickname FROM Board b")
