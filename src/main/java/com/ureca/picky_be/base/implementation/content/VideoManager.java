...
public class VideoManager {
    ...
    public List<String> uploadVideo(List<MultipartFile> multipartFiles) throws IOException {
        return multipartFiles.stream()
                .map(file -> {
                    try {
                        File uploadFile = multipartFileToFile(file)
                                .orElseThrow(() -> new CustomException(ErrorCode.CONTENT_CONVERT_FAILED));
                        return extractFilePath(uploadVideo(uploadFile));
                    } catch (IOException e) {
                        throw new CustomException(ErrorCode.BOARD_CONTENT_UPLOAD_FAILED);
                    }
                })
                .toList();
    }
    ...
    private Optional<File> multipartFileToFile(MultipartFile multipartFile) throws IOException {
        ...
        File file = new File(s3Config.getFileDir()  + "video/" + storeFileName);
    ...
    private String uploadVideo(File uploadFile) throws IOException {

        String fileName = "video/" + UUID.randomUUID() + uploadFile.getName();
        String uploadVideoUrl = putS3(uploadFile, fileName);
    ...
    private String putS3(File uploadFile, String fileName) throws IOException {
        ...
        try {
            s3Config.s3Client().putObject(putObjectRequest, uploadFile.toPath()); //uploaedFile.toPath(): 업로드할 로컬 파일의 경로
